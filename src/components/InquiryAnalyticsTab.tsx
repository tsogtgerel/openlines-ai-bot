...
export const InquiryAnalyticsTab: React.FC<InquiryAnalyticsTabProps> = ({
  ...
  // Agent Performance Time Filtering states
  const [agentTimeRange, setAgentTimeRange] = useState<
    'all' | 'today' | 'yesterday' | 'week' | 'month' | '30days' | 'custom'
  >('all');
  const [agentStartDate, setAgentStartDate] = useState('');
  const [agentEndDate, setAgentEndDate] = useState('');
  const [customAgentPerformance, setCustomAgentPerformance] = useState<AgentPerformanceStat[] | null>(null);
  const [isAgentPerfLoading, setIsAgentPerfLoading] = useState(false);
  const agentPerfCache = useRef<Map<string, AgentPerformanceStat[]>>(new Map());

  // Time Range definitions for UI
  const timeRangeOptions = useMemo(
    () => [
      { id: 'all' as const, label: 'Бүх хугацаа', desc: 'Бүртгэлтэй нийт чатууд' },
      { id: 'today' as const, label: 'Өнөөдөр', desc: 'Өнөөдрийн ээлжийн чатууд' },
      { id: 'yesterday' as const, label: 'Өчигдөр', desc: 'Өчигдрийн гүйцэтгэл' },
      { id: 'week' as const, label: 'Сүүлийн 7 хоног', desc: 'Сүүлийн 7 хоногийн чатууд' },
      { id: 'month' as const, label: 'Энэ сар', desc: 'Энэ сарын эхнээс хойш' },
      { id: '30days' as const, label: 'Сүүлийн 30 хоног', desc: 'Сүүлийн 30 хоногийн дүн' },
      { id: 'custom' as const, label: 'Тусгай огноо', desc: 'Сонгосон огнооны интервал' },
    ],
    []
  );

  const activeTimeRangeLabel = useMemo(() => {
    if (agentTimeRange === 'custom') {
      if (agentStartDate && agentEndDate) return `${agentStartDate} ~ ${agentEndDate}`;
      if (agentStartDate) return `${agentStartDate}-аас хойш`;
      if (agentEndDate) return `${agentEndDate} хүртэл`;
      return 'Тусгай огноо';
    }
    const found = timeRangeOptions.find((t) => t.id === agentTimeRange);
    return found ? found.label : 'Бүх хугацаа';
  }, [agentTimeRange, agentStartDate, agentEndDate, timeRangeOptions]);
  ...
  // Available channel filters - dynamically include openLines when available
  const computedChannelOptions = useMemo(() => {
    const base = [
      { id: 'all', label: 'Бүх сувгууд (Omnichannel)', icon: Globe },
    ];

    if (openLines && openLines.length > 0) {
      const lineOpts = openLines.map((line) => {
        let Icon = Globe;
        const nameLower = (line.name || '').toLowerCase();
        if (nameLower.includes('fb') || nameLower.includes('facebook') || nameLower.includes('мебель')) {
          Icon = MessageSquare;
        } else if (nameLower.includes('chat') || nameLower.includes('web') || nameLower.includes('их дэлгүүр')) {
          Icon = Radio;
        } else if (nameLower.includes('ig') || nameLower.includes('instagram')) {
          Icon = MessageSquare;
        } else if (nameLower.includes('tg') || nameLower.includes('telegram')) {
          Icon = Send;
        ...
        return {
          id: String(line.id),
          label: line.name || `Суваг #${line.id}`,
          icon: Icon,
        };
      });
      return [...base, ...lineOpts];
    }
  ...
  const fetchReport = async (channelsToFetch: string[] = selectedChannels, forceAi: boolean = false) => {
    ...
    // 1. Instant Cache Hit: 0ms UI update
    if (!forceAi && reportCache.current.has(sortedKey)) {
  ...
  useEffect(() => {
    fetchReport(selectedChannels, false);
  }, []);

  // Fetch filtered agent performance metrics (fast & cached)
  const fetchAgentPerformanceData = async (
    timeRange: string,
    startDate?: string,
    endDate?: string,
    channels: string[] = selectedChannels
  ) => {
    const isAllChannels = channels.includes('all');
    const chParam = isAllChannels ? '' : channels.join(',');
    const cacheKey = `${chParam}_${timeRange}_${startDate || ''}_${endDate || ''}`;
    ...
    try {
      setIsAgentPerfLoading(true);
      const params = new URLSearchParams();
      if (!isAllChannels && chParam) params.set('channels', chParam);
      if (timeRange && timeRange !== 'all') params.set('timeRange', timeRange);
      if (startDate) params.set('startDate', startDate);
      if (endDate) params.set('endDate', endDate);

      const res = await fetch(`/api/analytics/agent-performance?${params.toString()}`).then((r) => r.json());
      if (res.success && res.data) {
        agentPerfCache.current.set(cacheKey, res.data);
        setCustomAgentPerformance(res.data);
      }
    } catch (err) {
      console.error('Failed to fetch agent performance:', err);
    } finally {
      setIsAgentPerfLoading(false);
    }
  };
  ...
  const handleDownloadMarkdown = () => {
    ...
    const exportAgentStats = effectiveAgentPerformance;
    if (exportAgentStats && exportAgentStats.length > 0) {
      md += `\n## 5. Операторуудын Гүйцэтгэл & AI Ашиглалтын Үзүүлэлт (${activeTimeRangeLabel})\n\n`;
      md += `| Операторын Нэр | Албан тушаал | Нийт Чат | Дундаж Хариулах Хугацаа | AI Ашиглалт % | Үнэлгээ | Статус |\n`;
      md += `|---|---|---|---|---|---|---|\n`;
      exportAgentStats.forEach((a) => {
  ...
  // Effective agent performance array (either time-filtered custom or from base report)
  const effectiveAgentPerformance = useMemo(() => {
    if (customAgentPerformance !== null) {
      return customAgentPerformance;
    }
    return report?.agentPerformance || [];
  }, [customAgentPerformance, report?.agentPerformance]);
  ...
  const filteredAgents = useMemo(() => {
    if (!effectiveAgentPerformance) return [];
    let list = [...effectiveAgentPerformance];
    ...
    return list;
  }, [effectiveAgentPerformance, agentSearchQuery, agentStatusFilter, agentSortBy]);

  // Overall agent performance metrics summary
  const agentTeamSummary = useMemo(() => {
    if (!effectiveAgentPerformance || effectiveAgentPerformance.length === 0) {
      return {
        totalAgents: 0,
        activeAgentsCount: 0,
        totalChats: 0,
    ...
    const agents = effectiveAgentPerformance;
    const totalAgents = agents.length;
    const activeAgents = agents.filter((a) => a.totalChatsHandled > 0);
    const activeAgentsCount = activeAgents.length;
    const totalChats = agents.reduce((acc, a) => acc + a.totalChatsHandled, 0);

    const respondentsWithTime = activeAgents.filter((a) => a.avgResponseTimeSeconds > 0);
    const avgResponseSeconds =
      respondentsWithTime.length > 0
        ? Math.round(
            respondentsWithTime.reduce((acc, a) => acc + a.avgResponseTimeSeconds, 0) /
              respondentsWithTime.length
          )
        : 0;
    const avgResponseFormatted =
      avgResponseSeconds === 0
        ...
        ? `${avgResponseSeconds} сек`
        : `${(avgResponseSeconds / 60).toFixed(1)} мин`;

    const avgAiUsage =
      activeAgents.length > 0
        ? Math.round(activeAgents.reduce((acc, a) => acc + a.aiUsageRate, 0) / activeAgents.length)
        : 0;

    const topAgent =
      activeAgents.length > 0
        ? [...activeAgents].sort((a, b) => b.totalChatsHandled - a.totalChatsHandled)[0]
        : null;
    ...
    <div className="space-y-6 pb-12 print:space-y-4 print:pb-0" id="inquiry-analytics-container">
          ...
          <div className="flex flex-wrap items-center gap-2.5 shrink-0">
            <button
              ...
              onClick={() => {
                ...
                fetchReport(selectedChannels, true);
              }}
      ...
      <div className="bg-white rounded-xl p-3.5 sm:p-4 border border-slate-200/80 shadow-2xs space-y-3 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-1.5 pt-1">
          {computedChannelOptions.map((opt) => {
            const Icon = opt.icon;
        ...
        <div className="space-y-6" id="agent-performance-section">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-2xs space-y-1">
              ...
              <p className="text-[11px] text-slate-500">
                {agentTeamSummary.activeAgentsCount} оператор чатад ажилласан
              </p>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-2xs space-y-1">
              ...
              <p className="text-[11px] text-slate-500">
                {activeTimeRangeLabel} хариулсан чатууд
              </p>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-2xs space-y-3">
            ...
            {/* Time Period Filter Bar (Операторуудын гүйцэтгэлийг хугацаагаар шүүх) */}
            <div className="pt-2.5 border-t border-slate-100 space-y-2">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 text-xs text-slate-700 font-semibold">
                  <Calendar className="w-3.5 h-3.5 text-blue-600" />
                  <span>Хугацаагаар шүүх:</span>
                  {isAgentPerfLoading && (
                ...
                {agentTimeRange !== 'all' && (
                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[11px] font-semibold border border-blue-200/60">
                      <span>Идэвхтэй:</span>
                      <span>{activeTimeRangeLabel}</span>
                    </span>
                    <button
                      type="button"
                      onClick={() => handleAgentTimeRangeChange('all')}
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                {timeRangeOptions.map((opt) => {
                  const isSelected = agentTimeRange === opt.id;
                  return (
                    <button
                      key={opt.id}
                      type="button"
                      id={`time-range-btn-${opt.id}`}
                      onClick={() => handleAgentTimeRangeChange(opt.id)}
                      title={opt.desc}
                      className={`px-2.5 py-1 text-xs rounded-lg font-medium transition active:scale-95 border cursor-pointer ${
                        isSelected
                          ? 'bg-blue-600 text-white border-blue-600 shadow-2xs font-semibold'
                          : 'bg-slate-50 hover:bg-slate-100 text-slate-700 border-slate-200'
                      }`}
              ...
              {agentTimeRange === 'custom' && (
                <div className="flex flex-wrap items-center gap-2 p-2.5 rounded-lg bg-blue-50/60 border border-blue-200/60 text-xs">
                  <span className="text-blue-900 font-medium text-[11px]">Огнооны завсар:</span>
                  <div className="flex items-center gap-1.5">
                    <label htmlFor="agent-start-date" className="text-slate-600 text-[11px]">
                      Эхлэх:
                    </label>
                    <input
                      type="date"
                      ...
                      onChange={(e) => handleCustomDateChange(e.target.value, agentEndDate)}
                      className="px-2 py-1 text-xs bg-white border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-800"
                    />
                  </div>
                  <span className="text-slate-400 font-bold">-</span>
                  <div className="flex items-center gap-1.5">
                    <label htmlFor="agent-end-date" className="text-slate-600 text-[11px]">
                      Дуусах:
                    </label>
                    <input
                      type="date"
                      id="agent-end-date"
                      value={agentEndDate}
                      onChange={(e) => handleCustomDateChange(agentStartDate, e.target.value)}
                      className="px-2 py-1 text-xs bg-white border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-800"
                    />
                  </div>
                  {(agentStartDate || agentEndDate) && (
                    <button
                      type="button"
                      onClick={() => {
                        setAgentStartDate('');
                        setAgentEndDate('');
                        fetchAgentPerformanceData('custom', '', '', selectedChannels);
                      }}
                      className="px-2 py-1 text-[11px] text-slate-600 hover:text-slate-900 bg-white border border-slate-200 rounded-md cursor-pointer"
                    >
                      Огноо цэвэрлэх
                    </button>
                  )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5">
                            ...
                            {index === 0 && agent.totalChatsHandled > 0 && (
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-slate-50/70 p-3 rounded-xl border border-slate-100">
                      ...
                      <div className="space-y-0.5 text-center">
                        ...
                        <span className="text-[10px] text-emerald-700 font-semibold block">
                          {agent.resolvedChatsCount} шийдсэн ({agent.resolutionRate}%)
                        </span>
                      </div>
                    ...
                    <div className="pt-2 border-t border-slate-100 space-y-2 text-xs">
                      <div className="flex items-center justify-between text-[11px] text-slate-600">
                        <div className="flex items-center gap-1 font-semibold text-slate-800">
                          ...
                          <span>
                            {agent.totalChatsHandled > 0 ? `${agent.rating} / 5.0` : '- / 5.0'}
                          </span>
                          {agent.totalChatsHandled > 0 && (
                        ...
                        <span className="text-slate-500">
                          FCR: <strong>{agent.totalChatsHandled > 0 ? `${agent.firstContactResolutionRate}%` : '-'}</strong>
                        </span>
                      </div>
                      ...
                      >
                        ...
                        <td className="py-3 px-3 text-center">
                          ...
                          <span className="block text-[10px] text-slate-500">
                            {agent.totalChatsHandled === 0
                              ? 'Чат аваагүй'
                              : agent.avgResponseTimeSeconds <= 60
                              ? '⚡ <1 мин'
                              : '⏱️ хэвийн'}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-3 text-center">
                          <div className="inline-flex items-center gap-1 font-bold text-slate-800">
                            <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-400" />
                            <span>{agent.totalChatsHandled > 0 ? agent.rating : '-'}</span>
                          </div>
                          <span className="block text-[10px] text-slate-400">
                            {agent.totalChatsHandled > 0 ? `${agent.positiveSentimentRate}% эерэг` : 'Ээлжинд'}
                          </span>
                        </td>
